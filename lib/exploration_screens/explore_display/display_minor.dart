...
class _DisplayMinorState extends State<DisplayMinor> {
  ...
  void getAvgSalary() {
    if (minorWage != null) {
      String latestYr = minorWage!.data[0].year;
      int sumWage = 0;
      int count = 0;
      for (var eachWage in minorWage!.data) {
  ...
  void moreOrLess() {
    if (minorWage != null) {
      int cateSal = minorWage!.data[0].averageWage.toInt();
  ...
  void getTopMajs() {
    if (minorEdu != null) {
      String latestYr = minorEdu!.data[0].year;
      Map<int, String> allMajs = {};
      for (int i = 0; i < minorEdu!.data.length; i++) {
        if (minorEdu!.data[i].year == latestYr && minorEdu!=null) {
          allMajs[minorEdu!.data[i].totalPopulation] = minorEdu!.data[i].cip2;
        }
        else
          break;
      }
  ...
  void getMaxSkill() {
    if (minorSkill != null) {
      String latestYr = minorSkill!.data[0].year;
      double highestSkill = minorSkill!.data[0].lvValue.toDouble();
      double highestRca = minorSkill!.data[0].rca.toDouble();
      for (var eachSkill in minorSkill!.data) {
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  Text("In ${minorWage!.data[0].year.toString()}, ${widget.title} earned an "
                      "average yearly salary of \$${minorWage!.data[0].averageWage.toStringAsFixed(2)},"
                  ...
                  SfCartesianChart(
                    title: ChartTitle(text: "Average annual Salary in ${minorWage!.data[0].year}"),
                    tooltipBehavior: toolTipBehaviorWage,
                    series: <ChartSeries>[
                      BarSeries<YearlyWage, String>(
                          name: "Average annual salary in ${minorWage!.data[0].year}",
                  ...
                  Text("The revealed comparative advantage (RCA), defined in "
                      ...
                      style: TextStyle(fontSize: 16)),
                  SfCircularChart(
                    title: ChartTitle(text: "Skills Group Total LV Value in ${minorSkill!.data[0].year}"),
                  ...
                  SizedBox(
                    ...
                    child: SfCartesianChart(
                      title: ChartTitle(text: "Skills Element in ${minorSkill!.data[0].year}"),
                      tooltipBehavior: toolTipBehaviorSkillsElem,
                      series: <ChartSeries>[
                        BarSeries<SkillsElemFreq, String>(
                            name: "Skills Element Value in ${minorSkill!.data[0].year}",
  ...
  List<String>? getSkillsNameRadarChart() {
    if (minorSkill != null) {
      List<String> skills = [];
      String latestYear = minorSkill!.data[0].year.toString();
      for (var eachSkill in minorSkill!.data) {
  ...
  List<YearlyWage>? getYearlyWageBarChart() {
    if (minorWage != null) {
      ...
      String latestYear = minorWage!.data[0].year.toString();
      for (var eachWage in minorWage!.data) {
        if (latestYear == eachWage.year) {
          yearlyChart.add(YearlyWage(eachWage.minorOccupationGroup.substring(0,10), eachWage.averageWage.toInt()));
        }
        else {
          break;
        }
  ...
  List<EduFrequency> getEduTreeMapData() {
    ...
    topEduMajs?.forEach((key, value) {
      treeMap.add(EduFrequency(value, minorEdu!.data[0].year, key));
    });
